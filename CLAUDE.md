...
lint・formatter・テストスイートは設定されていません。`GEMINI_API_KEY` は環境変数からは読み込みません。利用者が費用発生タイミングを自分で把握・管理できるよう、**常にサイドバーで手入力する方式**に統一しています（`.env`は使用しません）。入力値は `st.session_state` にのみ保持され、ファイルには書き出されません。
...
- 本番デプロイ先は Streamlit Community Cloud（`https://7uygefsvdfkzcrxxjtbwqt.streamlit.app/`）。GitHubへのプッシュで自動再デプロイされますが、反映が遅れる場合は管理画面から「Reboot app」で手動リブートしてください。
- `.claude/skills/skill-creator`（実体は `.agents/skills/skill-creator`）は `npx skills add https://github.com/anthropics/skills --skill skill-creator` で導入したベンダー化スキルで、`.gitignore` 対象です。`skills-lock.json` があれば同じコマンドで再インストールできます。
