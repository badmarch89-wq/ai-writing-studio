...
## 5. Claude Code まわりの仕組み（CLAUDE.md / Skills / devcontainer）

本プロジェクトは Claude Code（AIコーディングエージェント）を使って開発したため、Claude Code特有の設定ファイルがいくつか入っている。それぞれ役割が違うので整理する。

### 5.1 `CLAUDE.md` ＝ このリポジトリの「ルール・引き継ぎメモ」

`CLAUDE.md` は、Claude Codeが**次に新しいセッションでこのリポジトリを開いたとき最初に読むファイル**。人間で言う「新人への引き継ぎ資料」に近い。

- **何を書くか**: よく使うコマンド（起動・依存関係インストールなど）、アーキテクチャの全体像（`app.py`/`tools.py`/`gemini_client.py`/`style.py`の役割分担）、新機能を追加するときに従うべきパターン（例: 新しいツールを追加する際は`render_*`関数を書いて`TOOLS`辞書に登録する、など）。
- **役割**: ここに書かれた内容が事実上「このリポジトリのルール」として機能する。Claude Codeはコードを書く前にこのルールに従おうとするため、内容が古い・間違っていると誤った実装をしてしまう。**大きな仕様変更をしたら`CLAUDE.md`も一緒に更新する**のが望ましい。
- **作り方**: `/init` コマンドで、既存コードを解析して自動生成できる（本プロジェクトもこれで作成）。日本語で欲しい場合は `/init は日本語にして修正してください` のように依頼すれば書き直してくれる。

### 5.2 Skills（スキル）＝ 特定作業の手順書をClaude Codeに追加する仕組み

**Skill**とは、「特定の種類のタスクをどう進めるか」を記述した手順書（Markdownファイル + 補助スクリプト）で、Claude Codeに追加インストールすることで `/スキル名` のスラッシュコマンドとして呼び出せるようになる機能。

- 例: 本プロジェクトでは `streamlit-llm-security-check` というスキルを自作し、`/streamlit-llm-security-check` でAPIキー漏洩・プロンプトインジェクション・XSSなどのセキュリティ観点を毎回同じ手順でチェックできるようにした。
- スキルは **プロジェクト単位**（`.claude/skills/`、そのリポジトリでのみ使える）と、**グローバル**（`~/.claude/skills/`、全プロジェクトで使える）の2箇所に配置できる。本プロジェクトで作った `streamlit-llm-security-check` はグローバル側に配置されている。

### 5.3 `skill-creator`：スキルを作るためのスキル

`skill-creator` は、Anthropic公式リポジトリ（`anthropics/skills`）で配布されている「スキルを作るためのスキル」。これを使うと、対話しながら新しいスキルを設計・作成・改善できる。
...
- `npx skills` は npm パッケージ `skills` を実行してGitHubリポジトリから指定スキルを取得・配置するCLIツール。
- 実行すると、プロジェクト直下に以下が作られる。
  - `.agents/skills/skill-creator/` … スキル本体（実ファイル）
  - `.claude/skills/skill-creator` … 上記へのシンボリックリンク（Claude Codeが読みに行く場所）
  - `skills-lock.json` … どのスキルをどこから入れたか記録するロックファイル（このファイルさえあれば同じスキルを再インストールできる）
- **注意**: インストール直後の会話ではまだ認識されないことがある。Claude Codeはセッション開始時にスキル一覧を読み込むため、**新しいセッションを開始（再起動）して初めて `/skill-creator` が使えるようになる。**
- `.agents/` と `.claude/skills/skill-creator` は「ベンダー化された（外部から取り込んだ）ツール一式」として `.gitignore` に含めている。`skills-lock.json` さえコミットしておけば、`npx skills add` を再実行して誰でも同じ状態を復元できるため、大量の外部ファイルをリポジトリに含める必要がない。

**利用方法（実際の流れ）:**

1. `/skill-creator` を実行
2. 「このアプリケーションをセキュリティチェックするスキルを作ってください」のように、作りたいスキルの内容を伝える
3. skill-creatorが対話的にスキルの草案（`SKILL.md`）を作成
4. 「SKILL.mdは日本語でお願いします」のように言語や書式を指定して調整
5. 完成したスキルは（プロジェクト用かグローバル用かを選んで）配置される
6. 以後は `/streamlit-llm-security-check` のように、作ったスキル名をスラッシュコマンドとして呼び出せる

### 5.4 `.devcontainer` ＝ GitHub Codespaces用の環境定義（再掲）

3.7節の通り、GitHub上のWeb UI操作で追加された。ローカルの `.venv` とは別に、ブラウザ上のGitHub Codespacesで開いたときの環境構築を自動化する設定。`skill-creator`や`CLAUDE.md`とは無関係の、実行環境そのものの定義ファイルである点に注意（コンテナのベースイメージ、起動時に実行するコマンドなどを記述する）。

---

## 6. 注意事項

### 6.1 APIキー漏洩リスクの確認ポイント
...
### 6.4 ドキュメントは実施内容だけを書く

「〇〇と連携した」と書く前に、実際に設定ファイルや接続が存在するか確認してから記載する（例: 本プロジェクトではNetlify連携について聞かれた際、実際には未設定だったため「未連携」と明記した）。憶測でデプロイ済みと書かない。

### 6.5 `CLAUDE.md` は仕様変更のたびに更新する

`CLAUDE.md`はClaude Codeが毎回参照する「ルール」なので、内容が古いまま放置すると、廃止したはずの実装パターン（例: APIキーの環境変数自動読み込み）に沿ったコードを再度書かれてしまうことがある。大きめの仕様変更をしたら、コードだけでなく`CLAUDE.md`も合わせて見直す。
...
- 2026-09-04: 「5. Claude Codeまわりの仕組み」章を新設し、`CLAUDE.md`の役割、Skills機能、`skill-creator`のダウンロード〜利用方法、`.devcontainer`との違いを解説。あわせて`CLAUDE.md`本体もAPIキー方式変更などの最新状態に合わせて更新
